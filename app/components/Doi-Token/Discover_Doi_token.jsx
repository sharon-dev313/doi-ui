  ...
  return (
   <div className={`max-w-7xl mx-auto px-4  sm:px-6 mt-[150px] lg:px-8 flex justify-center ${!SubCards ? "gap-[150px]" : "gap-[24px]"} `} >
     ...
     <span className='text-[#04B9E8]' >{review}</span>
             ...
             {title}
              <p className="text-lg mt-5 text-default-600 max-w-lg">
                {paragraph}
              </p>
                
               {SubCards && (
                 <Button variant="solid"  radius="full" size="lg" className="font-nasa w-[212px] mt-10 bg-[#04B9E8] text-black">
                  Buy Tokens
                </Button>
                )}
          {!SubCards && !FAQ ? (
            ...
            <div className='flex gap-8 mt-12 ' >
             
              {
                sweepstakesCountData?.map((item , index) => (
                    ...
                    <div key={item.id} className={`w-full ${index !== sweepstakesCountData.length - 1 ? "border-r border-default-200" : ""}   `} >
                      <h1 className='text-3xl leading-[34px] font-nasa' >{item.value}</h1>
           ...
           {!SubCards && (
                 <Button variant="solid" color="primary" radius="full" size="lg" className="font-nasa w-xs mt-auto text-black">
                  View More
                </Button>
                )}
            </>
                ...
                <div className="flex gap-[24px] mt-auto" >
                        ...
                        <Card className={"!w-full bg-[#F3F5F9] h-[244px] shadow-none  !border-none"}   radius="lg">
...
{/* Second Card  */}
                        ...
                        <Card className={"!w-full bg-[#E6F8FD] shadow-none h-[244px] border-none"} isFooterBlurred  radius="lg">
      ...
      <CardFooter className=" flex flex-col items-start overflow-hidden ps-6 shadow-none py-6 absolute before:rounded-xl rounded-large bottom-1 w-[calc(100%_-_8px)]  ml-1 z-10">
       ...
       <p> Booking Discount</p>
          ...
          <h1 className='text-2xl font-nasa leading-[34px]' >Still have a question?</h1>
          <p className='text-default-600' >Don’t worry we’re free for consultation, just click button below.</p>
        
        <Button variant="solid" color="primary" radius="full" size="lg" className="font-nasa w-xs mt-10 text-black">
                  Any Question
                </Button>
          </div>
            ) : null }
...
{ SubCards && !FAQ && (
          ...
          <div className="flex gap-[24px] " >
              <Card radius="lg" className="w-[424px] h-fit shadow-none border-none p-0">
  <CardBody className="p-0 ">
    ...
    />
                       ...
                       <Card radius="lg" className="w-[312px] shadow-none h-full border-none p-0">
  <CardBody className="p-0 ">
    <img
      alt="House on a couch"
src={ImageItems2}
          ...
          <div className="flex w-[580px] flex-col mt-8 border gap-[76px] rounded-3xl bg-[#e6f8fd] p-8" >
            {ListItems.map((item , index) =>  (
              <div key={index} className="flex   gap-[15px]" >
                ...
                <h3 className="text-xl  font-nasa text-default-800" >{item.Title}</h3>
